...
import voya.domain.RegionOption;
...
public class OptionsValidator {
	...
	public Map<String, String> loadAllRegionOptions() throws IOException {
		...
		String[] nameMapping = new String[] { "option", "type" };
		CellProcessor[] processors = new CellProcessor[] {
				new NotNull(), // option
				new NotNull()  // optionType
		};

		initalizeBeanReader(fileName);
		RegionOption regionOption;

		try {
			while ((regionOption = beanReader.read(RegionOption.class, nameMapping,
					processors)) != null) {
				allRegionOptions.put(regionOption.getOption(), regionOption.getType());
			}
		} catch (IOException e) {
